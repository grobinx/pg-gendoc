...
begin
  return jsonb_agg(
           jsonb_build_object(
             'table_name', c.relname, 'description', d.description, 
             'kind', case c.relkind when 'f'::char then 'foreign' when 'r'::char then 'ordinary' end,
    ...
    from pg_class c
         ...
         join lateral 
              (select jsonb_agg(
                        jsonb_build_object(
                          'column_no', column_no, 'data_type', data_type, 'nullable', nullable, 
                          'default_value', default_value, 'storage_type', storage_type, 
                          'description', description, 'collation_name', collation_name, 
